...
class WhoisReplyListener(ReplyListener):
    ...
    def notify(self, client, event):
        if event.command == "RPL_WHOISUSER":
            ...
            reply = self.WhoisReplyEvent()
            reply.lcnick = event.params[0].lower()
            reply.nick = event.params[0]
            reply.user = event.params[1] 
            reply.host = event.params[2] 
            reply.real_name = event.params[4]
            self._whois_replies[event.params[0].lower()] = reply
        elif event.command == "RPL_WHOISCHANNELS":
            # <nick> :*( ( "@" / "+" ) <channel> " " )
            channels = event.params[1].strip().split()
            channels = map(protocol.strip_name_symbol, channels)
            self._whois_replies[event.params[0].lower()].channels.extend(channels)
        elif event.command == "RPL_WHOISSERVER":
            # <nick> <server> :<server info> 
            self._whois_replies[event.params[0].lower()].server = event.params[1]
        elif event.command == "RPL_WHOISACCOUNT":
            self._whois_replies[event.params[0].lower()].account = event.params[1]
            self._whois_replies[event.params[0].lower()].identified = True
        elif event.command == "RPL_WHOISIDLE":
            # <nick> <integer> :seconds idle
            self._whois_replies[event.params[0].lower()].idle_time = event.params[1]
            ...
            try:
                rpl = self._whois_replies[event.params[0].lower()]
            except KeyError:
                return
            self.activate_handlers(client, self._whois_replies[event.params[0].lower()])
            del self._whois_replies[event.params[0].lower()]
